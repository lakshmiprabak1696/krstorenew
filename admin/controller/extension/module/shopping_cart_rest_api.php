...
class ControllerExtensionModuleShoppingCartRestApi extends Controller
{
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
                 ...
                 $this->config->set('module_shopping_cart_rest_api', 1);
    ...
    public function install() {
         ...
         $this->db->query(" SET @DropColumnIfExist = 
        (  
	SELECT IF
	(
		(
			...
			WHERE table_name = '". DB_PREFIX  ."address'
                        and table_schema='".DB_DATABASE."' 
			AND column_name = 'phone'
		) = 0,
		\"ALTER TABLE oc_address ADD COLUMN  phone VARCHAR(255) NULL after lastname;\",
             \"SELECT 'YourColumnName already exist in YourTableName table.'\" 
	)
        );
        ");
        
        $this->db->query(" PREPARE alterIfNotExists FROM @DropColumnIfExist;");
        $this->db->query(" EXECUTE alterIfNotExists;");
        $this->db->query(" DEALLOCATE PREPARE alterIfNotExists; ");
	...
	(
		...
		\"ALTER TABLE `". DB_PREFIX  ."order` ADD COLUMN  shipping_phone VARCHAR(191) NULL AFTER shipping_code;;\",
             \"SELECT 'YourColumnName already exist in YourTableName table.'\" 
	)
        );
        ");
	...
	(
		...
		\"ALTER TABLE `". DB_PREFIX  ."order` ADD COLUMN payment_phone VARCHAR(191) NULL AFTER shipping_code;\",
             \"SELECT 'YourColumnName already exist in YourTableName table.'\" 
	)
        );
        ");
	...
	(
		(
			...
			WHERE table_name = '". DB_PREFIX  ."product'
                        and table_schema='".DB_DATABASE."' 
			AND column_name = 'TopUpYN'
		) = 0,
		\"ALTER TABLE `". DB_PREFIX  ."product` ADD `TopUpYN` TINYINT(1) NOT NULL DEFAULT '0' AFTER `date_modified`;\",
             \"SELECT 'YourColumnName already exist in YourTableName table.'\" 
	)
        );
        ");
