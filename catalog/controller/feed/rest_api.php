    ...
    */
    ...
    private function getProductInfo($product, $simpleList = false, $customFields = false)
    {
        ...
        if (!$simpleList) {
            ...
            $reviewList = $this->model_catalog_review->getReviewsByProductId($product['product_id'], 0, 5);
    ...
    public function listProducts($category_id, $request)
    {
        ...
        if (isset($request->get['topUp']) && !empty($request->get['topUp'])) {
            $parameters["filter_topUp"] = $request->get['topUp'];
        }
        ...
        if (isset($this->request->get['option_value'])) {
            ...
            $parameters['filter_sub_category'] = true;
            $parameters['options'] = array_keys($option_values);
        }else
            $option_values = array();
...
//            );
    ...
    public function listCategories($parent, $level )
    {
        ...
        $data = $this->loadCatTree($parent, 15);
...
//            );
     ...
     public function getCurrency()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            $this->load->model('catalog/product');
            $this->load->model('localisation/currency');
            $results = $this->model_localisation_currency->getCurrencyByCode($this->currency->getRestCurrencyCode());
                    
             $this->json['data']['currency_code']= $this->currency->getRestCurrencyCode();;
             $this->json['data']['symbol_left']= $results['symbol_left'];
             $this->json['data']['symbol_right']= $results['symbol_right'];
...
//            );
    ...
    public function reviews()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($this->request->get['start']) && ctype_digit($this->request->get['start']) && $this->request->get['start'] > 0) {
                $start = $this->request->get['start'];
            }
            
            if (isset($this->request->get['product_id']) && ctype_digit($this->request->get['product_id'])) {
                 $this->json['data']  =$this->getReview($this->request->get['product_id'],$start,$limit);
            } else {
                 $this->statusCode = 400;
                $this->json['error'][] = "Invalid identifier.";
            }
     ...
     public function getReview($product_id,$start,$limit )
    {
        /*reviews*/
        $this->load->model('catalog/review');

        $reviews["review_total"] = $this->model_catalog_review->getTotalReviewsByProductId($product_id);

        $reviewList = $this->model_catalog_review->getReviewsByProductId($product_id, $start, $limit);
...
//            );
    ...
    public function featured()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'GET') {
            ...
            $this->getFeaturedProducts($limit,$page);
        } else {
            $this->statusCode = 405;
            $this->allowedHeaders = array("GET");
        }
    ...
    public function getFeaturedProducts($limit,$page)
    {
        ...
        if (count($featureds)) {
            ...
            foreach ($featureds as $featured) {
                ...
                if(isset($settings['product']))
                    ...
                    if ($limit) {
                        $products = array_slice($products, $pa, (int)$limit);
                    }
                        ...
                        foreach ($all as $product_info) {
                            ...
                            if ($product_info) {
                                ...
                                $item = array(
                                    ...
                                    'wish_list' => $product_info['wish_list'],
                                    'thumb' => $image,
...
//            );
    ...
    private function getBestsellers($request)
    {
        ...
        $results = $this->model_catalog_product->getBestSellerProductsWithStart($page,$limit,$this->customer);
    ...
    public function listBanners() {
        ...
        if (!empty($banners)) {
            ...
            foreach ($banners as &$banner) {
                $data = $this->getBanner($banner['banner_id']);
