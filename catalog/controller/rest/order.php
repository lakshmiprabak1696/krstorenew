     ...
     public function updateorderstatus() {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($this->request->get['order_id'])) {
                ...
                $this->model_account_order->updateOrderStatus($order_id);
                $this->json['sucess'][] = "Update Sucess ";
                $this->statusCode = 200;
            } else {
                $this->json['error'][] = "Missing ID";
                $this->statusCode = 400;
        ...
        if (empty($this->json['error'])) {
            ...
            if ($order_info) {
                ...
                foreach ($products as $product) {
                    ...
                    if (isset($product_info['image']) && !empty($product_info['image']) && file_exists(DIR_IMAGE . $product_info['image'])) {
                        ...
                        $original_image = $this->urlPrefix . 'image/' . $product_info['image'];
                    } else {
            ...
            $start = ($start - 1) * $limit;
              ...
              $parameters[]=array();
                if (isset($this->request->get['status']) && !empty($this->request->get['status'])) { 
                    $parameters["status"] = $this->request->get['status'];
                    
                }
                

		$results = $this->model_account_order->getOrdersWithFilter($start, $limit,$parameters);
