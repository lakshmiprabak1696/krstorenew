    ...
    public function saveOrderToDatabase() {
        ...
        if (isset($this->session->data['order_id'])) {
            ...
            if (isset($this->session->data['order_id'])) {
                ...
                $wallet_code = $this->model_setting_setting->getSettingValue('module_wallet_payment_method', $store_id);

                if ($this->session->data['payment_method']['code'] == $wallet_code) {
                    $wallet_data = array(
                        "customer_id" => $this->customer->getId(),
                        'order_id' => $this->session->data['order_id'],
                        'order_product_id' => 0,
                        'amount' => $this->session->data['total'] * (-1),
                        'type' => 'Purchase'
                    );
        ...
        if (empty($this->json['error'])) {
            ...
            if (empty($this->json['error'])) {
                ...
                if ($this->cart->hasShipping()) {
                    ...
                    $order_data['shipping_phone'] = $shipping_address['phone'];
                ...
                $this->session->data['total'] = $order_data['total'];
